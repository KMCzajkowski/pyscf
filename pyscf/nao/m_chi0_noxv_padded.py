...
def chi0_mv(self, dvin, comega=1j*0.0, nmsel="none", dnout="none"):
    """
    ...
    """
    if dnout=="none": dnout = np.zeros_like(dvin, dtype=self.dtypeComplex)
          ...
          for m,(em,fm) in enumerate(zip(self.ksn2e[0,s,vs:],self.ksn2f[0,s,vs:])):
            ...
            if nmsel is None:
              ...
              if [n,m] in nmsel:
                ...
                nm2v_re[n, m] = nm2v.real*0
                nm2v_im[n, m] = nm2v.imag*0              
...
def chi0_mv_split(self, dvin, comega=1j*0.0, nmsel="none", dnout="none"):
    """
        Apply the non-interacting response function to a vector
        Input Parameters:
        -----------------
    ...
    if dnout=="none": dnout = np.zeros((self.nspin,self.nprod,self.ksn2e[0,0,:nf].shape[0],self.ksn2e[0,0,vs:].shape[0]), dtype=self.dtypeComplex)

    sp2v  = dvin.reshape((self.nspin,self.nprod))
    #print(nmsel)    
    sp2dn = dnout
    #.reshape((self.nspin,self.nprod,self.ksn2e[0,s,:nf].shape[0],self.ksn2e[0,s,vs:].shape[0]))
    ...
    for s in range(self.nspin):
      ...
      for n,(en,fn) in enumerate(zip(self.ksn2e[0,s,:nf], self.ksn2f[0,s,:nf])):
        for m,(em,fm) in enumerate(zip(self.ksn2e[0,s,vs:],self.ksn2f[0,s,vs:])):
          ...
          sp2dn[s,:,n,m] = chi0_re + 1.0j*chi0_im
