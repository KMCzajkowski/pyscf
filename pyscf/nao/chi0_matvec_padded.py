...
from pyscf.nao.m_chi0_noxv_padded import chi0_mv_gpu, chi0_mv, chi0_mv_split
...
class chi0_matvec(mf):
  ...
  def apply_rf0(self, sp2v, comega=1j*0.0, nmsel=None):
    ...
    if self.td_GPU.GPU is None:
      return chi0_mv(self, sp2v, comega, nmsel)
    else:
      return chi0_mv_gpu(self, sp2v, comega)

  def apply_rf0_split(self, sp2v, comega=1j*0.0):
    """ This applies the non-interacting response function to a vector (a set of vectors?) """
    expect_shape=tuple([self.nspin*self.nprod])
    assert np.all(sp2v.shape==expect_shape), "{} {}".format(sp2v.shape,expect_shape)
    self.rf0_ncalls+=1
    ...
    if self.td_GPU.GPU is None:
      return chi0_mv_split(self, sp2v, comega)
    else:
      return chi0_mv_gpu(self, sp2v, comega)
  ...
  def comp_polariz_nonin_xx_atom_split(self, comegas):
    """  Compute the non interacting polarizability along the xx direction split into contributions from individual atoms."""
